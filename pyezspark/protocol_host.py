...
class postTunnelRun(threading.Thread):
    def __init__(self, gym_server_url, training_public_key, training_private_key, neat, gym_game_name, buffer_size,
                 genome_input, genome_output, url_name, remote_ip, remote_port, genomes_per_client):
        threading.Thread.__init__(self)
        ...
        self.client = ezclient.Client(self.training_public_key, training_private_key=self.training_private_key,
                                      neat_class=self.neat, gym_game_name=self.gym_game_name,
                                      buffer_size=self.buffer_size, genome_input=self.input_size,
                                      genome_output=self.output_size, url_name=self.url_name)
...
class Host:
    def __init__(self,
                 gym_game_name, alone_training_iterations, configuration_dict,
                 max_number_of_games, max_number_of_steps, training_public_key=None, training_private_key=None,
                 socket1_training_public_key=None,
                 socket1_training_private_key=None, socket2_training_public_key=None, socket2_training_private_key=None,
                 socket3_training_public_key=None,
        ...
        if max_number_of_steps <= 0 or max_number_of_games <= 0 or alone_training_iterations < 0:
            print(
                "Error, something among max number of steps, max number of games or alone training iterations is <= 0")
            exit(1)
        ezclient.get_randomness()
        self.t_val = t_val
        self.gym_game_name = "CartPole-v0"
        ...
        self.neat = ezclient.Neat(self.configuration_dict['input_size'], self.configuration_dict['output_size'],
                                  initial_population=configuration_dict['initial_population'],
                                  species_threshold=configuration_dict['species_threshold'],
                                  max_population=configuration_dict['max_population'],
                                  generations=gen,
                                  percentage_survivors_per_specie=configuration_dict['percentage_survivors_per_specie'],
                                  connection_mutation_rate=configuration_dict['connection_mutation_rate'],
                                  new_connection_assignment_rate=configuration_dict['new_connection_assignment_rate'],
                                  add_connection_big_specie_rate=configuration_dict['add_connection_big_specie_rate'],
                                  add_connection_small_specie_rate=configuration_dict[
                                      'add_connection_small_specie_rate'],
                                  add_node_specie_rate=configuration_dict['add_node_specie_rate'],
                                  activate_connection_rate=configuration_dict['activate_connection_rate'],
                                  remove_connection_rate=configuration_dict['remove_connection_rate'],
                                  children=configuration_dict['children'],
                                  crossover_rate=configuration_dict['crossover_rate'],
                                  saving=configuration_dict['saving'],
                                  limiting_species=configuration_dict['limiting_species'],
                                  limiting_threshold=configuration_dict['limiting_threshold'],
                                  same_fitness_limit=configuration_dict['same_fitness_limit'],
                                  keep_parents=configuration_dict['keep_parents'],
                                  age_significance=configuration_dict['age_significance'])
        self.alone_training_iterations = 100
    ...
    def alone_training_with_bayesian(self, **configuration_dict):
        neat = ezclient.Neat(self.configuration_dict['input_size'], self.configuration_dict['output_size'],
                             initial_population=self.configuration_dict['initial_population'],
                             species_threshold=configuration_dict['species_threshold'],
                             max_population=self.configuration_dict['max_population'],
                             generations=10,
                             percentage_survivors_per_specie=configuration_dict['percentage_survivors_per_specie'],
                             connection_mutation_rate=configuration_dict['connection_mutation_rate'],
                             new_connection_assignment_rate=configuration_dict['new_connection_assignment_rate'],
                             add_connection_big_specie_rate=configuration_dict['add_connection_big_specie_rate'],
                             add_connection_small_specie_rate=configuration_dict['add_connection_small_specie_rate'],
                             add_node_specie_rate=configuration_dict['add_node_specie_rate'],
                             activate_connection_rate=configuration_dict['activate_connection_rate'],
                             remove_connection_rate=configuration_dict['remove_connection_rate'],
                             children=self.configuration_dict['children'],
                             crossover_rate=configuration_dict['crossover_rate'],
                             saving=self.configuration_dict['saving'],
                             limiting_species=self.configuration_dict['limiting_species'],
                             limiting_threshold=self.configuration_dict['limiting_threshold'],
                             same_fitness_limit=configuration_dict['same_fitness_limit'],
                             keep_parents=self.configuration_dict['keep_parents'],
                             age_significance=configuration_dict['age_significance'])

        env = gym.make(self.configuration_dict['gym_game_name'])

        best_fitness = 0

        for i in range(10):
            neat.set_generation_iter(i)
            number_genomes = neat.get_number_of_genomes()
            neat.reset_fitnesses()

            for j in range(number_genomes):
                state = env.reset()
                state = gym_http_server.flat_state(state[0])
                n_games = 0
                steps = 0

                fitness = 0
                for k in range(self.configuration_dict['max_number_of_steps']):
                    steps += 1
                    l = [state]
                    indices = [j]
                    out = neat.ff_ith_genomes(l, indices, 1)
                    m = -1
                    ind = -1

                    for ij in range(len(out[0])):
                        if out[0][ij] > m:
                            m = out[0][ij]
                            ind = ij

                    es = env.step(ind)
                    [state, reward, done] = es[0], es[1], es[2]
                    fitness += reward
                    state = gym_http_server.flat_state(state)
                    neat.increment_fitness_of_genome_ith(j, reward)

                    if done or steps >= self.configuration_dict['max_number_of_steps']:
                        n_games += 1
                        steps = 0
                        state = env.reset()
                        state = gym_http_server.flat_state(state[0])

                    if n_games >= self.configuration_dict['max_number_of_games']:
                        fitness /= n_games
    ...
    def bayesian(self):
        ...
        pbounds = {
            'species_threshold': (0, 10),
            'percentage_survivors_per_specie': (0, 1),
            'connection_mutation_rate': (0, 1),
            'new_connection_assignment_rate': (0, 1),
            'add_connection_big_specie_rate': (0, 1),
            'add_connection_small_specie_rate': (0, 1),
            'add_node_specie_rate': (0, 1),
            'activate_connection_rate': (0, 1),
            'remove_connection_rate': (0, 1),
            'crossover_rate': (0, 1),
            'same_fitness_limit': (0, 50),
            'age_significance': (0, 1)
        }

        optimizer = BayesianOptimization(
            f=self.alone_training_with_bayesian,
            pbounds=pbounds,
            verbose=2,
            random_state=1
        )
    ...
    def distributed_training(self, remote_ip, remote_port, genomes_per_client, max_number_of_trainers, ip='127.0.0.1',
                             port=5000, timeout=3):
        ...
        self.client = ezclient.Client(self.training_public_key, training_private_key=self.training_private_key,
                                      neat_class=self.neat, gym_game_name=self.gym_game_name, buffer_size=30000,
                                      genome_input=self.input_size, genome_output=self.output_size, url_name=link)
        ...
        socket1_thread = postTunnelRun('http://' + ip + ':' + str(port), self.socket1_training_public_key,
                                       self.socket1_training_private_key,
                                       self.neat, self.gym_game_name, 30000, self.input_size, self.output_size, link,
                                       remote_ip, remote_port, genomes_per_client)
        socket2_thread = postTunnelRun('http://' + ip + ':' + str(port), self.socket2_training_public_key,
                                       self.socket2_training_private_key,
                                       self.neat, self.gym_game_name, 30000, self.input_size, self.output_size, link,
                                       remote_ip, remote_port, genomes_per_client)
        socket3_thread = postTunnelRun('http://' + ip + ':' + str(port), self.socket3_training_public_key,
                                       self.socket3_training_private_key,
                                       self.neat, self.gym_game_name, 30000, self.input_size, self.output_size, link,
                                       remote_ip, remote_port, genomes_per_client)
        ...
        while (True):
            ...
            if self.client.trainer_has_identifier():
                ...
                if environment_name in gym_http_server.glob_val.reverse_shared_d or current_id not in gym_http_server.glob_val.id_p2p or self.client.get_number_of_fitnesses() != \
                        gym_http_server.glob_val.id_p2p[current_id]['n_genomes']:
                    is_ok = False
            ...
            if is_ok:  # is ok as trainer, or is malicious but did not relly hurted us, or is ok but timeout timed out it
                if self.client.set_body(1, gym_http_server.glob_val.current_index, gym_http_server.glob_val.next_index,
                                        flag2):  # if is true, a generation run has been completed
                ...
                if current_id != None:
                    if current_id in gym_http_server.glob_val.id_p2p:
                        ...
                        gym_http_server.glob_val.exit_critical_section()
                self.client.set_body(0, gym_http_server.glob_val.current_index, gym_http_server.glob_val.next_index,
                                     False)  # we set the body to tell the server to close this trainer
            # unlocking timeouts
            gym_http_server.glob_val.enter_critical_section()
